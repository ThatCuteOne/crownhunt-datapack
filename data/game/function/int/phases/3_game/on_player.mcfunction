...
execute if entity @s[tag=!selected_kit,tag=!spec,tag=game.in_game] run function game:int/kit_loading/give_random with entity @e[tag=kit.selector,limit=1,sort=random] ArmorItems[3].components.minecraft:custom_data
execute if entity @s[tag=game.in_game,tag=insert_player,tag=!selected_kit] run function game:int/kit_loading/give_random with entity @e[tag=kit.selector,limit=1,sort=random] ArmorItems[3].components.minecraft:custom_data
